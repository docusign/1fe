...
export const getParsedWidgetUrlOverrides = (): Record<string, string> => {
    try {
        ...
        console.error(
        `[1DS][RUNTIME_CONFIG_OVERRIDES] Unable to parse JSON in ${WIDGET_URL_OVERRIDES} param`,
        );
...
export const isAllowedSource = (source: string, allowedSources: string[]): boolean => {
    try {
