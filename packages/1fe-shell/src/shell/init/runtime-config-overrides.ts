...
import {
  ...
  getRuntimeConfigUrlFromBundleUrl,
  overrideWidgetConfigRuntime,
} from '../utils/runtime-configs';
import { getShellPlatformUtils } from '../utils/shell-platform-utils';
import {
  getParsedWidgetUrlOverrides,
  isAllowedSource,
} from '../utils/widget-url-overrides';

/**
 * The "current" runtime config for any environment can be replaced in two ways:
 * 1. By passing the runtime_config_overrides query param. This is usually done automatically by `yarn 1fe-cli dev --overrideRuntimeConfig`
  ...
  Object.entries(importMapOverrides)
    .filter(([widgetId, url]) => {
      ...
      return (
        ...
        WIDGET_CONFIGS.get(widgetId)
      ); // exclude non-widget overrides, e.g. @ds/ui
    })
    .forEach(([widgetId]) => {
      ...
      try {
        ...
        console.error(
          `[1FE] Failed to apply runtime config override for widget ${widgetId}`,
          error,
        );
      }
    });
  ...
  const runtimeConfigFetches = widgetUrlOverridesMissingFromLocalStorage.map(
    async (widgetId) => {
      ...
      try {
        ...
        console.error(
          `[1FE] Failed to fetch runtime config for widget ${widgetId} from ${runtimeConfigUrl}`,
          error,
        );
      }
    },
