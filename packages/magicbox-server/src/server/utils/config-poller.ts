...
import { generateWidgetConfigMap, getCachedWidgetConfigs, getWidgetConfigValues, mapAndGenerateWidgetConfigMap, setCachedWidgetConfigs } from './widget-config';
import { fetchAllWidgetRuntimeConfigs, getFallbackRuntimeConfigs } from './runtime-configs';
...
const verifyWidgetCDNUrls = async (
    widgetConfigsToVerify: WidgetConfigs,
  ) => {
...
const pollDynamicWidgetConfig = async (url: string): Promise<void> => {
    try {
        ...
        const widgetConfigsPayload = (result as any)?.cdn?.widgets?.releaseConfig || [];
        ...
        if (didWidgetVersionsUpdate(previousConfigs, widgetConfigMap)) {
          const newWidgetConfigs = await fetchAllWidgetRuntimeConfigs(widgetConfigMap);

          // Construct widget cdn urls and ensure they return 200
          const widgetCdnUrlsNotVerified = await verifyWidgetCDNUrls(
              newWidgetConfigs,
          );
          ...
          if (widgetCdnUrlsNotVerified.length === 0 || SKIP_WIDGET_CDN_VERIFICATION) {
        ...
        setCachedWidgetConfigs(mapAndGenerateWidgetConfigMap(
            widgetConfigMap,
