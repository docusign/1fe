...
import { PreloadType, RuntimeConfig, WidgetConfig, WidgetConfigs } from "../types";
import { generateWidgetConfigMap, getCachedWidgetConfigs, getWidgetConfigValues } from "./widget-config";
...
const getIsColdStart = (): boolean => {
    return !Array.from(getCachedWidgetConfigs().values()).some((widgetConfig) =>  widgetConfig.runtime !== undefined);
  }
...
const parseRuntimeConfig = ({
    ...
    if (parsedRuntimeConfig?.preload) {
      const parsedPreloads = parsedRuntimeConfig.preload.map((preloadObj: PreloadType) => {
...
const _fetchSingleWidgetRuntimeConfig = async ({
    ...
    if (response?.status === 200) {
      ...
      const message = '[DYNAMIC_CONFIG][WIDGETS][CRITICAL] Failed to fetch runtime config for widget with 5xx during cold start. Retrying infinitely.'
  ...
  export const fetchAllWidgetRuntimeConfigs = async (
    ...
    const updatedWidgetConfigs = await Promise.all(
        ...
        try {
            ...
            const newWidgetConfig = await _fetchSingleWidgetRuntimeConfig({ widgetConfig });
